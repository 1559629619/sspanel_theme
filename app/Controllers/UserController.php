 ...
 */ 
        ...
        return $this->view()->assign("ssr_sub_token", $ssr_sub_token)->assign("router_token", $router_token)
            ...
            ->assign("user", $this->user)->registerClass("URL", "App\Utils\URL")->assign('baseUrl', Config::get('baseUrl'))->assign("userip", $userip)->assign("userloginip", $userloginip)->assign("paybacks", $paybacks)->display('user/index.tpl');
    }
...
Telegram::Send("OMG，" . $this->user->user_name . " 大老爷给我们捐了 " . $codeq->number . " 元呢~");
           /** if (Config::get('enable_donate') == 'true') {
    ...
    public function buy($request, $response, $args)
    {
        ...
        $res['ret'] = 1;
        $res['msg'] = "购买成功";
      Telegram::Send("上车成功，" . $this->user->user_name . " 大老爷购买了 \n$" . $shop->name . "$ \n让我们为他欢呼吧~~~");
        return $response->getBody()->write(json_encode($res));
    }
